...
const STATUS_LABELS: Record<'DRAFT' | 'SENT' | 'ARCHIVED', string> = {
  DRAFT: 'Entwurf',
  SENT: 'Versendet',
  ARCHIVED: 'Archiv',
}
...
export default function OfferTrackingPage({ params }: { params: { id: string } }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const trackRes = await fetch(`/api/admin/pitches/${params.id}/tracking${qs}`)
      if (trackRes.ok) {
  ...
  async function openDetail(viewId: string) {
    ...
    try {
      const res = await fetch(`/api/admin/pitches/${params.id}/tracking?viewId=${viewId}`)
      if (res.ok) {
  ...
  return (
    <AdminShell>
      <div className={styles.pageHeader}>
        <Link href="/admin/pitches" className={styles.backLink}>
          <IconArrowLeft />
          Zur Übersicht
        </Link>
        ...
        {data?.pitch && (
          <div className={styles.pageSub}>
            {data.pitch.clientCompany}
            {data.pitch.occasion ? ` · ${data.pitch.occasion}` : ''}{' '}
            <Link href={`/p/${data.pitch.slug}`} target="_blank" className={styles.inlineLink}>
              <IconExternalLink /> Pitch ansehen
            </Link>
          </div>
        )}
      </div>
      ...
      {!loading && data && data.stats.sessionCount === 0 && (
        <div className={styles.emptyState}>
          Noch keine Aufrufe registriert. Sobald jemand das Pitch öffnet, erscheinen
          die Daten hier.
        </div>
      )}
